import type { InternalRequest, RawFile } from "../RequestManager.ts";
...
function isErrorResponse(
  ...
  return (
    typeof Reflect.get(error as Record<string, unknown>, "message") === "string"
  );
}
...
export class DiscordAPIError extends Error {
  ...
  private static *flattenDiscordError(
    ...
    if (isErrorResponse(obj)) {
      return yield `${key.length ? `${key}[${obj.code}]` : `${obj.code}`}: ${
        obj.message
      }`.trim();
    }
