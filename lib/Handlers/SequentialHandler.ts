...
export class SequentialHandler implements IHandler {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/explicit-member-accessibility
  #sublimitPromise: { promise: Promise<void>; resolve: () => void } | null =
    null;
  ...
  public get inactive(): boolean {
    return (
      ...
      (this.#sublimitedQueue === null ||
        this.#sublimitedQueue.remaining === 0) &&
      !this.limited
    );
  }
  ...
  public async queueRequest(
    ...
    if (
      this.#sublimitedQueue &&
      hasSublimit(routeId.bucketRoute, requestData.body, options.method)
    ) {
  ...
  private async runRequest(
    ...
    while (this.limited) {
      ...
      if (isGlobal) {
        this.debug(
          `Global rate limit hit, blocking all requests for ${timeout}ms`
        );
      } else {
    ...
    try {
      ...
      if (
        ...
        retries !== this.manager.options.retries
      ) {
    ...
    this.reset = reset
      ? Number(reset) * 1000 + Date.now() + this.manager.options.offset
      : Date.now();
