...
export class RequestManager extends EventEmitter {
  ...
  private setupSweepers() {
    ...
    if (
      ...
      this.options.hashSweepInterval !== Infinity
    ) {
      ...
      this.hashTimer = setInterval(() => {
        ...
        this.hashes.sweep((v, k) => {
          ...
          const shouldSweep =
            Math.floor(currentDate - v.lastAccess) > this.options.hashLifetime;
          ...
          this.emit(
            RESTEvents.Debug,
            `Hash ${v.value} for ${k} swept due to lifetime being exceeded`
          );
    ...
    if (
      ...
      this.options.handlerSweepInterval !== Infinity
    ) {
      ...
      this.handlerTimer = setInterval(() => {
        ...
        this.handlers.sweep((v, k) => {
          ...
          this.emit(
            RESTEvents.Debug,
            `Handler ${v.id} for ${k} swept due to being inactive`
          );
          return inactive;
        });
  ...
  public async queueRequest(request: InternalRequest): Promise<unknown> {
    ...
    const hash = this.hashes.get(
      `${request.method}:${routeId.bucketRoute}`
    ) ?? {
  ...
  private static generateRouteData(
    ...
    const majorIdMatch = /^\/(?:channels|guilds|webhooks)\/(\d{16,19})/.exec(
      endpoint
    );
    ...
    if (
      ...
      baseRoute === "/channels/:id/messages/:id"
    ) {
